...
import { getVersions, deleteVersions } from './versionFeature';
    ...
    return (
        <main>
            ...
            >
                ...
                <Link to="/admin/add-version" className="admin_add_btn">
                                        ...
                                        state.loading ? (
                                            ...
                                            data.map((retunData, index) => (
                                                <tr key={retunData._id}>
                                                    <td>{index + 1}</td>
                                                    <td>{retunData.name}</td>
                                                    <td>{retunData.catId?.name || 'N/A'}</td>
                                                    <td>{retunData.makeId?.name || 'N/A'}</td>
                                                    <td>{retunData.modelId?.name || 'N/A'}</td>
                                                    <td>
                                                        <div className="d-flex gap-2">
                                                            <button
                                                                className="btn btn-sm btn-outline-primary"
                                                                onClick={() => navigate(`/admin/edit-version/${retunData._id}`)}
                                                            >
                                                                ✎ Edit
                                                            </button>
                                                            <button
